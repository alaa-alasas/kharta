...
const WorkIndex = () => {
  ...
  setLoading(true);
  try {
    const res = await axios.get<ApiResponse>("http://localhost:3000/api/works", {
      params: {
  ...
  const handleConfirrmDelete = () => {
    axios
      .delete(`http://localhost:3000/api/works/${id}`, {
        headers: {
  ...
  return (
    <div className="items-page-content d-flex flex-column align-items-center container-lg">
      {/* حقل البحث */}
      <SearchInput
        placeholder="البحث باسم المهنة"
        classExtra="pb-5"
        onSearch={handleSearch}
      />

      <BtnCustom
        name="إضافة مهنة جديدة"
        ...
        <table className="table" style={{ width: '60vw' }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={4} className="text-center">لا يوجد مهن</td>
              </tr>
            ) : (
              works.map((work, index) => (
                <tr key={work.id}>
                  ...
                  <td>{work.name}</td>
                  <td>
                    <Link to={`edit/${work.id}`}><MdEdit /></Link>
                    <button className="btn btn-link p-0 text-danger" onClick={() => handleShow(work.id)}>
                      <MdDelete size={20} />
                    </button>
                  </td>
                </tr>
