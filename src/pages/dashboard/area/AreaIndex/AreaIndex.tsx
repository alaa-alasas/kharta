...
const AreaIndex = () => {
  ...
  setLoading(true);
  try {
    const res = await axios.get<ApiResponse>("http://localhost:3000/api/areas", {
      params: {
  ...
  const handleConfirrmDelete = () => {
    axios
      .delete(`http://localhost:3000/api/areas/${id}`, {
        headers: {
  ...
  return (
    <div className="items-page-content d-flex flex-column align-items-center container-lg">
      {/* حقل البحث */}
      <SearchInput
        placeholder="البحث باسم المنطقة أو المحافظة"
        classExtra="pb-5"
        ...
        <table className="table" style={{ width: '60vw' }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={4} className="text-center">لا يوجد مناطق</td>
              </tr>
            ) : (
              areas.map((area, index) => (
                <tr key={area.id}>
                  ...
                  <td>{area.name}</td>
                  <td>
                    <Link to={`edit/${area.id}`}><MdEdit /></Link>
                    <button className="btn btn-link p-0 text-danger" onClick={() => handleShow(area.id)}>
                      <MdDelete size={20} />
                    </button>
                  </td>
                </tr>
